...
function drawRulesNotice(textLines) {
  rectMode(CENTER);
  textAlign(CENTER, CENTER);
  fill(0, 0, 0, 180);
  ...
  fill(C_WHITE);
  textSize(24);
  text(textLines, width/2, height/2 - 20);
  fill(C_BLUE);
  rect(width/2, height/2 + height * 0.2 - 30, 220, 55, 12);
  fill(255);
  textSize(22);
  text('開始遊戲', width/2, height/2 + height * 0.2 - 30);
}
...
function drawLevel1() {
  ...
  text(`目前得分: ${levelScore} | 剩餘時間: ${max(0, (10 - elapsed).toFixed(1))}s`, width/2, 80);
...
function drawLevel3() {
  ...
  if (elapsed < 2) {
    ...
    // 自動落下
    fill(255, 100, 100); // 粉色
    let dropY = ballY + (elapsed - 12) * 500;
